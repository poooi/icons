...
The subsequent inclination correction concerns rotation about the compass's own centre-to-loop axis, not rotation in the image plane. The transverse projected axis is shortened while the centre-to-loop axis is held fixed; the rear case moves toward the lower left and the side crown stays at half depth. The loop's extension is shortened from 24.5 to 20.8 local units, keeping its attachment points at the rim. Enlarged light/dark comparisons were reviewed alongside the source, followed by small-size and rendered-bound checks. The source bitmap does not determine an exact 3D angle, so the adjustment remains a visual reconstruction.
